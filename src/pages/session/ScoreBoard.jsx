  ...
  return (
    <div className={classes} style={{ direction: isEn ? "ltr" : "rtl" }}>
      {scoreBoard && scoreBoard.length === 0 && (
        <div>
          <p className="no-scores-yet">
            {isEn ? "No Scores yet..." : "...אין תוצאות עדיין"}
          </p>
        </div>
      )}
      <motion.table
        whileHover={inReport ? { scale: 1 } : { scale: 1.03 }}
        className="scoreboard-list"
        onClick={clickOnScoreboard}
      >
        {scoreBoard && scoreBoard.length > 0 && (
          <tr className="scoreboard-header">
            {inModal && <th style={{ width: "5vw" }}> </th>}
            <th style={inModal ? { width: "6vw" } : { width: "1.5vw" }}>
              {inModal ? (isEn ? "Rank" : "דירוג") : isEn ? "R" : "דירוג"}
            </th>
            <th style={inModal ? { width: "6vw" } : { width: "1.5vw" }}>
              {inModal ? (isEn ? "Score" : "ניקוד") : isEn ? "S" : "ניקוד"}
            </th>
            <th style={inModal ? { width: "17vw" } : { width: "5vw" }}>
              {isEn ? "Name" : "שם"}
            </th>
            <th style={inModal ? { width: "12vw" } : { width: "3vw" }}>
              {inModal
                ? isEn
                  ? "Wins/Games"
                  : "משחקים/נצחונות"
        ...
        {scoreBoard &&
          scoreBoard.length > 0 &&
          allClubPlayersDocs &&
          scoreBoard.slice(0, maxShow).map((score, i) => {
            ...
            return (
              ...
              >
                ...
                <td>{score.place}</td>
                <td>{score.score}</td>
                {!handlePlayerClick && <td>{showName(player)}</td>}
                {handlePlayerClick && <td onClick={() => handlePlayerClick(player.id)}>
                  {showName(player)}
                </td>}
